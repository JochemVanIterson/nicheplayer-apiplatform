<template lang="pug">
  q-page(padding)
    ...
    q-list(bordered separator class="rounded-borders")
      q-item(v-for="album in filteredAlbums" v-if="album" :clickable="albumAllowed(album)" :disabled="!albumAllowed(album)" @click="albumClicked(album)")
  ...
  computed: {
    ...
    filteredAlbums() {
      return this.payments.map((e) => {
        let album = this.albums[e.album.replace("/api/albums/", "")]
        if (album && album != "collecting")
          album.payment = e
        return album
      }).sort((a, b) => {
